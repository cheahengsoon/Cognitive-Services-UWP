...
namespace CogsExplorer.Modules.ComputerVision.Helpers
{
    ...
    {
        public async static Task<StorageFile> GetFileFromUriAsync(Uri uri, string fileName)
        {
            IRandomAccessStreamReference thumbnail = RandomAccessStreamReference.CreateFromUri(uri);
            StorageFile file = await StorageFile.CreateStreamedFileFromUriAsync(fileName, uri, thumbnail);
        ...
        public async static Task<Image.ImageAnalysisResult> GetImageAnalysisAsync(Guid id, byte[] bytes)
        {
            ...
            var results = await client.PostAsync(new Uri($"{CogsExplorer.Common.CoreConstants.CognitiveServicesBaseUrl}/vision/v1.0/analyze?visualFeatures={analysisFeatures}"), payload);
        ...
        public async static Task<Ocr.OcrAnalysisResult> GetOcrAnalysisAsync(Guid id, byte[] bytes)
        {
            ...
            var results = await client.PostAsync(new Uri($"{CogsExplorer.Common.CoreConstants.CognitiveServicesBaseUrl}/vision/v1.0/ocr"), payload);
        ...
        public async static Task<Handwriting.Operations.HandwritingOperationResult> GetHandwritingAnalysisAsync(Guid id, byte[] bytes)
        {
            ...
            var response = await client.PostAsync(new Uri($"{CogsExplorer.Common.CoreConstants.CognitiveServicesBaseUrl}/vision/v1.0/recognizeText?handwriting=true"), payload);
